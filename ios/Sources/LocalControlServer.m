...
static void SBControlHandleClient(int clientFd, NSString *remoteAddress) {
    ...
    if (strcmp(method, "OPTIONS") == 0) {
        ...
        SBControlSendJSON(clientFd, 404, @"Not Found", @{
            @"ok": @NO,
            @"error": @"supported endpoints: GET /health, GET/POST /ready, POST /battle-stage-probe, POST /battle-finish-body"
        });
    }
...
static void SBControlStartServerNow(void) {
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
        ...
        SBControlAppendIndexLine(@"control-listening",
                                 [NSString stringWithFormat:@"0.0.0.0:%u endpoints=/health,/ready,/battle-stage-probe,/battle-finish-body", SBControlPort]);
