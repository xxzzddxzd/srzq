...
#endif
static const uint16_t SBControlDefaultPort = (uint16_t)SB_CONTROL_PORT;
static NSString * const SBControlPortDefaultsKey = @"SoccerAppBypass.control.port";
static uint16_t SBControlPort = (uint16_t)SB_CONTROL_PORT;
static const NSTimeInterval SBControlFallbackStartDelaySeconds = 30.0;
static atomic_bool SBControlServerStarted = false;
static atomic_int SBControlServerFd = -1;
static atomic_uint SBControlServerGeneration = 1;
...
static NSDictionary *SBControlSwitchServerPort(uint16_t port);
...
static BOOL SBControlPortIsValid(NSInteger port) {
    return port >= 1024 && port <= 65535;
}

static uint16_t SBControlLoadConfiguredPort(void) {
    NSUserDefaults *defaults = NSUserDefaults.standardUserDefaults;
    id stored = [defaults objectForKey:(NSString *)SBControlPortDefaultsKey];
    if (!stored) {
        return SBControlDefaultPort;
    }

    NSInteger port = [defaults integerForKey:(NSString *)SBControlPortDefaultsKey];
    if (!SBControlPortIsValid(port)) {
        return SBControlDefaultPort;
    }
    return (uint16_t)port;
}

static void SBControlSaveConfiguredPort(uint16_t port) {
    NSUserDefaults *defaults = NSUserDefaults.standardUserDefaults;
    [defaults setInteger:port forKey:(NSString *)SBControlPortDefaultsKey];
    [defaults synchronize];
}

static void SBControlResetConfiguredPort(void) {
    NSUserDefaults *defaults = NSUserDefaults.standardUserDefaults;
    [defaults removeObjectForKey:(NSString *)SBControlPortDefaultsKey];
    [defaults synchronize];
}
...
static void SBControlPresentSettings(void);
...
- (void)openSettings:(__unused UIButton *)button {
    SBControlPresentSettings();
}
...
static NSString *SBControlEndpointSummary(void) {
    ...
    uint16_t configuredPort = SBControlLoadConfiguredPort();
    ...
    NSString *summary = [NSString stringWithFormat:@"State: %@\nListen: 0.0.0.0:%u\nLAN: %@\nProxy/local: 127.0.0.1:%u",
                         state,
                         SBControlPort,
                         lan,
                         SBControlPort];
    if (configuredPort != SBControlPort) {
        summary = [summary stringByAppendingFormat:@"\nNext launch: 0.0.0.0:%u", configuredPort];
    }
    return summary;
}

static BOOL SBControlParsePortString(NSString *text, uint16_t *outPort) {
    ...
    long long value = trimmed.longLongValue;
    if (!SBControlPortIsValid((NSInteger)value)) {
        return NO;
    }
    if (outPort) {
...
static NSDictionary *SBControlSettingsPayload(BOOL saved, NSString *message) {
    uint16_t configuredPort = SBControlLoadConfiguredPort();
    NSMutableDictionary *payload = [NSMutableDictionary dictionary];
    payload[@"ok"] = @YES;
    payload[@"service"] = @"SoccerAppBypass";
    payload[@"activePort"] = @(SBControlPort);
    payload[@"port"] = @(SBControlPort);
    payload[@"configuredPort"] = @(configuredPort);
    payload[@"defaultPort"] = @(SBControlDefaultPort);
    payload[@"restartRequired"] = @(configuredPort != SBControlPort);
    payload[@"serverFd"] = @(atomic_load(&SBControlServerFd));
    payload[@"saved"] = @(saved);
    if (message.length > 0) {
...
static NSDictionary *SBControlUpdateSettingsFromBody(NSData *bodyData) {
    if (bodyData.length == 0) {
        return @{
            @"ok": @NO,
            @"error": @"POST body must be {\"port\":19877} or {\"reset\":true}"
        };
    }
    ...
    if ([json isKindOfClass:NSDictionary.class]) {
        ...
        id reset = dict[@"reset"] ?: dict[@"Reset"];
        if ([reset respondsToSelector:@selector(boolValue)] && [reset boolValue]) {
            return SBControlSwitchServerPort(SBControlDefaultPort);
        }

        id value = dict[@"port"] ?: dict[@"Port"] ?: dict[@"controlPort"] ?: dict[@"ControlPort"];
        if ([value respondsToSelector:@selector(stringValue)]) {
            uint16_t port = 0;
            if (SBControlParsePortString([value stringValue], &port)) {
                return SBControlSwitchServerPort(port);
            }
        }
    }
    ...
    if (!json && jsonError) {
        ...
        uint16_t port = 0;
        if (SBControlParsePortString(raw, &port)) {
            return SBControlSwitchServerPort(port);
        }
    }
    ...
    return @{
        @"ok": @NO,
        @"error": @"invalid settings body; use port 1024..65535"
    };
}

static void SBControlPresentMessage(NSString *title, NSString *message) {
    SBControlRunOnMainRunLoop(^{
        UIViewController *controller = SBControlTopViewController();
        if (!controller) {
            return;
        }
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:title ?: @"SoccerAppBypass"
                                                                       message:message ?: @""
...
static void SBControlPresentSettings(void) {
    SBControlRunOnMainRunLoop(^{
        ...
        uint16_t configuredPort = SBControlLoadConfiguredPort();
        NSString *message = [NSString stringWithFormat:@"Active port: %u\nDefault port: %u\nSaving a port restarts only this local control server.",
                             SBControlPort,
                             SBControlDefaultPort];
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"SoccerAppBypass Settings"
                                                                       message:message
                                                                preferredStyle:UIAlertControllerStyleAlert];
        [alert addTextFieldWithConfigurationHandler:^(UITextField *textField) {
            textField.placeholder = [NSString stringWithFormat:@"%u", SBControlDefaultPort];
            textField.text = [NSString stringWithFormat:@"%u", configuredPort];
        ...
        [alert addAction:[UIAlertAction actionWithTitle:@"Save"
            ...
            uint16_t port = 0;
            if (!SBControlParsePortString(field.text ?: @"", &port)) {
                SBControlPresentMessage(@"Invalid port", @"Use a port from 1024 to 65535.");
                return;
            }

            NSDictionary *result = SBControlSwitchServerPort(port);
            if (![result[@"ok"] boolValue]) {
                SBControlPresentMessage(@"Port change failed", result[@"error"] ?: @"Unable to restart server on that port.");
            }
        }]];
        [alert addAction:[UIAlertAction actionWithTitle:@"Reset"
                                                  style:UIAlertActionStyleDestructive
                                                handler:^(__unused UIAlertAction *action) {
            NSDictionary *result = SBControlSwitchServerPort(SBControlDefaultPort);
            if (![result[@"ok"] boolValue]) {
                SBControlPresentMessage(@"Port reset failed", result[@"error"] ?: @"Unable to restart server on the default port.");
            }
        }]];
...
static void SBControlLayoutStatusPanel(UIWindow *window) {
    ...
    CGFloat width = MAX(260.0, CGRectGetWidth(frame));
    CGFloat height = MAX(220.0, CGRectGetHeight(frame));
    CGFloat contentX = 12.0 + insets.left;
    CGFloat contentY = 72.0 + insets.top;
    CGFloat contentWidth = MAX(120.0, width - contentX - 12.0 - insets.right);
    CGFloat textTop = contentY + 112.0;
    CGFloat textHeight = MAX(80.0, height - textTop - 12.0 - insets.bottom);
    ...
    UILabel *title = [[UILabel alloc] initWithFrame:CGRectMake(contentX, contentY, contentWidth - 44.0, 20.0)];
    ...
    settings.frame = CGRectMake(width - insets.right - 48.0, contentY - 6.0, 36.0, 32.0);
    ...
    if (@available(iOS 13.0, *)) {
        ...
        if (image) {
            ...
            [settings setTitle:@"SET" forState:UIControlStateNormal];
        }
    } else {
        [settings setTitle:@"SET" forState:UIControlStateNormal];
    }
    [settings setTitleColor:UIColor.whiteColor forState:UIControlStateNormal];
    settings.titleLabel.font = [UIFont boldSystemFontOfSize:11.0];
    [panel addSubview:settings];

    UILabel *endpoint = [[UILabel alloc] initWithFrame:CGRectMake(contentX, contentY + 24.0, contentWidth, 80.0)];
    endpoint.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    endpoint.numberOfLines = 5;
    ...
    UITextView *textView = [[UITextView alloc] initWithFrame:CGRectMake(contentX, textTop, contentWidth, textHeight)];
    ...
    [settings addTarget:SBControlStatusOverlayTargetObject
...
static void SBControlRefreshStatusOverlay(void) {
    SBControlRunOnMainRunLoop(^{
        ...
        SBControlStatusPanel.frame = window.bounds;
        SBControlStatusPanel.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
...
static void SBControlHandleClient(int clientFd, NSString *remoteAddress) {
    ...
    if (strcmp(method, "OPTIONS") == 0) {
        ...
        SBControlSendJSON(clientFd, 200, @"OK", @{
            ...
            @"restartRequired": @(configuredPort != SBControlPort)
        });
    } else if (strcmp(method, "GET") == 0 && [path isEqualToString:@"/control-settings"]) {
        SBControlSendJSON(clientFd, 200, @"OK", SBControlSettingsPayload(NO, @""));
    } else if (strcmp(method, "POST") == 0 && [path isEqualToString:@"/control-settings"]) {
        NSDictionary *settings = SBControlUpdateSettingsFromBody(SBControlBodyFromRequest(requestData));
        BOOL ok = [settings[@"ok"] boolValue];
        SBControlSendJSON(clientFd, ok ? 200 : 400, ok ? @"OK" : @"Bad Request", settings);
...
static int SBControlCreateServerSocket(uint16_t port, NSString **errorMessage) {
    int serverFd = socket(AF_INET, SOCK_STREAM, 0);
    if (serverFd < 0) {
        if (errorMessage) {
            *errorMessage = [NSString stringWithFormat:@"socket errno=%d", errno];
        }
        return -1;
    }
    ...
    if (bind(serverFd, (struct sockaddr *)&address, sizeof(address)) != 0) {
        ...
        close(serverFd);
        if (errorMessage) {
            *errorMessage = [NSString stringWithFormat:@"bind 0.0.0.0:%u errno=%d",
                             port,
                             savedErrno];
        }
        return -1;
    ...
    if (listen(serverFd, 8) != 0) {
        ...
        close(serverFd);
        if (errorMessage) {
            *errorMessage = [NSString stringWithFormat:@"listen errno=%d", savedErrno];
        }
        return -1;
    }
...
static void SBControlCloseServerFd(int serverFd) {
    if (serverFd < 0) {
        return;
    }
...
static void SBControlAcceptLoop(int serverFd, unsigned int generation) {
    dispatch_queue_t clientQueue = dispatch_queue_create("local.srzq.SoccerAppBypass.control.clients",
                                                         DISPATCH_QUEUE_CONCURRENT);
    while (true) {
        if (atomic_load(&SBControlServerGeneration) != generation ||
            atomic_load(&SBControlServerFd) != serverFd) {
            break;
        }
        ...
        if (ready < 0) {
            ...
            SBControlAppendIndexLine(@"control-select-error",
...
static NSDictionary *SBControlSwitchServerPort(uint16_t port) {
    if (!SBControlPortIsValid((NSInteger)port)) {
        return @{
            @"ok": @NO,
            @"error": @"invalid port; use 1024..65535",
            @"activePort": @(SBControlPort),
            @"configuredPort": @(SBControlLoadConfiguredPort())
        };
    }

    if (port == SBControlPort && atomic_load(&SBControlServerFd) >= 0) {
        SBControlSaveConfiguredPort(port);
        SBControlAppendStatusEvent([NSString stringWithFormat:@"SET port=%u already active", port]);
        SBControlRefreshStatusOverlay();
        return SBControlSettingsPayload(YES, @"port already active");
    }

    NSString *error = nil;
    int newFd = SBControlCreateServerSocket(port, &error);
    if (newFd < 0) {
        NSString *detail = error ?: @"unknown listen error";
        SBControlAppendStatusEvent([NSString stringWithFormat:@"SET port=%u failed: %@", port, detail]);
        SBControlAppendIndexLine(@"control-restart-failed",
                                 [NSString stringWithFormat:@"target=0.0.0.0:%u %@", port, detail]);
        SBControlRefreshStatusOverlay();
        return @{
            @"ok": @NO,
            @"error": detail,
    ...
    unsigned int generation = atomic_fetch_add(&SBControlServerGeneration, 1) + 1;
    int oldFd = atomic_exchange(&SBControlServerFd, newFd);
    uint16_t oldPort = SBControlPort;
    SBControlPort = port;
    SBControlSaveConfiguredPort(port);
    atomic_store(&SBControlServerStarted, true);

    SBControlAppendStatusEvent([NSString stringWithFormat:@"SET port=%u active", port]);
    SBControlAppendIndexLine(@"control-restarted",
                             [NSString stringWithFormat:@"old=0.0.0.0:%u new=0.0.0.0:%u",
                              oldPort,
                              port]);
    SBControlBeginAcceptLoop(newFd, port, generation);
    if (oldFd >= 0) {
        SBControlCloseServerFd(oldFd);
    }
    SBControlRefreshStatusOverlay();
    return SBControlSettingsPayload(YES, @"port saved and server restarted");
}
...
static void SBControlStartServerNow(void) {
    ...
    SBControlPort = SBControlLoadConfiguredPort();
    SBControlSetServerState(@"starting", [NSString stringWithFormat:@"port=%u", SBControlPort]);

    dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
        NSString *error = nil;
        int serverFd = SBControlCreateServerSocket(SBControlPort, &error);
        if (serverFd < 0) {
            NSString *detail = error ?: @"unknown listen error";
            SBControlSetServerState(@"failed", detail);
            SBControlAppendIndexLine(@"control-listen-failed",
                                     detail);
            atomic_store(&SBControlServerStarted, false);
            return;
        }

        unsigned int generation = atomic_fetch_add(&SBControlServerGeneration, 1) + 1;
        atomic_store(&SBControlServerFd, serverFd);
        SBControlBeginAcceptLoop(serverFd, SBControlPort, generation);
    });
}
